...
config = load_hparams('config_2DH.json')
...
#--------------- PDE parameters ---------------------------------------
a1 = config["PDE_hparams"]["a1"] #Source parameter a1
a2 = config["PDE_hparams"]["a2"] #Source parameter a2
...
class PeriodicC0Layer(keras.layers.Layer): #Implementation of the Fourier layer
    ...
    def call(self, inputs):
        ...
        yper = 2*np.pi*tf.matmul(y,ks[None,:])/Ly
        xcos = tf.math.cos(xper)
        xsin = tf.math.sin(xper)
        ycos = tf.math.cos(yper)
        ysin = tf.math.sin(yper)
        Xper = tf.concat([tf.concat([xcos,xsin],axis=1),tf.concat([ycos,ysin],axis=1)],axis=1)
        return Xper
...
def get_results(N,X):
   ...
   q = -(a1*np.pi)**2*tf.math.sin(a1*np.pi*x)*tf.math.sin(a2*np.pi*y)-\
        (a2*np.pi)**2*tf.math.sin(a1*np.pi*x)*tf.math.sin(a2*np.pi*y)+\
        k**2*tf.math.sin(a1*np.pi*x)*tf.math.sin(a2*np.pi*y)
        
   fu = u_xx + u_yy + k**2*u[:,0] -q
...
np.savetxt(f"loss_adam_2DH_{a1}_{a2}_{k}.txt",np.c_[epochs,loss_list])
...
def loss_and_gradient(weights,N,X,use_sqrt,use_log):
    '''
    Parameters
    ----------
    weights : NUMPY ARRAY
...
uexact = tf.math.sin(a1*np.pi*Xtest[:,0,None])*tf.math.sin(a2*np.pi*Xtest[:,1,None])
Xtest = convert_to_tensor(Xtest)
uexact = uexact.numpy().reshape(x.shape)

cont=0
def callback(*,intermediate_result): 
    global N,cont,lossbfgs,Nprint_bfgs,x,Xtest,uexact,error_list
    if (cont+1)%Nprint_bfgs == 0 or cont == 0:
        ...
        utest = output(N, Xtest).numpy().reshape(x.shape)
        error = np.linalg.norm(utest-uexact)/np.linalg.norm(uexact)
        error_list[(cont+1)//Nprint_bfgs] = error
        
        _,fuval = get_results(N,Xtest)
        validation_value = loss(fuval).numpy()
        validation_list[(cont+1)//Nprint_bfgs] = validation_value

        print(loss_value,error,validation_value,cont+1)
    cont+=1
...
if use_sqrt:
    fname_loss = f"2DH_{a1}_{a2}_{k}_loss_{method}_{method_bfgs}_sqrt.txt"
    fname_error = f"2DH_{a1}_{a2}_{k}_error_{method}_{method_bfgs}_sqrt.txt"
    fname_val = f"2DH_{a1}_{a2}_{k}_validation_{method}_{method_bfgs}_sqrt.txt"
elif use_log:
    fname_loss = f"2DH_{a1}_{a2}_{k}_loss_{method}_{method_bfgs}_log.txt"
    fname_error = f"2DH_{a1}_{a2}_{k}_error_{method}_{method_bfgs}_log.txt"
    fname_val = f"2DH_{a1}_{a2}_{k}_validation_{method}_{method_bfgs}_log.txt"
else:
    fname_loss = f"2DH_{a1}_{a2}_{k}_loss_{method}_{method_bfgs}.txt"
    fname_error = f"2DH_{a1}_{a2}_{k}_error_{method}_{method_bfgs}.txt"
    fname_val = f"2DH_{a1}_{a2}_{k}_validation_{method}_{method_bfgs}.txt"
    
np.savetxt(fname_loss,np.c_[epochs_bfgs,lossbfgs])
np.savetxt(fname_error,np.c_[epochs_bfgs,error_list])
np.savetxt(fname_val,np.c_[epochs_bfgs,validation_list])
