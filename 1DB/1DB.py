# -*- coding: utf-8 -*-
"""
Created on Wed Dec 11 10:54:50 2024
...
config = load_hparams('config_1DB.json')
...
def output(N,X):
    ...
    x = X[:,1,None]
    Nout = N(X)
    u = -tf.math.sin(2*np.pi*x/Lx) + t*Nout[:,0,None]
    return u
...
np.savetxt("loss_adam_1DB.txt",np.c_[epochs,loss_list])
...
def callback(*,intermediate_result): 
    global N,cont,lossbfgs,X_star,x_star,Nprint_bfgs,u_ref
    if (cont+1)%Nprint_bfgs == 0 or cont == 0:
        ...
        error = np.linalg.norm(u_ref-utest)/np.linalg.norm(utest)
...
if use_sqrt:
    fname_loss = f"1DB_loss_{method}_{method_bfgs}_sqrt.txt"
    fname_error = f"1DB_error_{method}_{method_bfgs}_sqrt.txt"
elif use_log:
    fname_loss = f"1DB_loss_{method}_{method_bfgs}_log.txt"
    fname_error = f"1DB_error_{method}_{method_bfgs}_log.txt"
else:
    fname_loss = f"1DB_loss_{method}_{method_bfgs}.txt"
    fname_error = f"1DB_error_{method}_{method_bfgs}.txt"
